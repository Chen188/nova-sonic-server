...
export class NovaSonicBidirectionalStreamClient {
  ...
  constructor(config: NovaSonicBidirectionalStreamClientConfig) {
    // Store the configs for potential updates later
    this.clientConfig = { ...config.clientConfig };
    this.requestHandlerConfig = config.requestHandlerConfig || {
  ...
  /**
   * Update the client's AWS credentials and region
   * @param credentials - New AWS credentials
   * @param region - New AWS region (optional)
   * @returns True if update was successful
   */
  public updateCredentials(credentials: any, region?: string): boolean {
    try {
      ...
      // Create a new HTTP handler with the existing config
      const nodeHttp2Handler = new NodeHttp2Handler({
        ...this.requestHandlerConfig,
      });

      // Create a new Bedrock client with the updated credentials and region
      this.bedrockRuntimeClient = new BedrockRuntimeClient({
        ...this.clientConfig,
        credentials: credentials,
        region: region || this.clientConfig.region || "us-east-1",
        requestHandler: nodeHttp2Handler
      });

      console.log(`Credentials and region updated successfully. New region: ${region || this.clientConfig.region}`);
      return true;
    } catch (error) {
    ...
    try {
      for await (const event of response.body) {
        ...
        if (event.chunk?.bytes) {
          ...
          console.error(`Validation error for session ${sessionId}: `, event.validationException);

          // Check if this is the "No open content found" error
          const errorMessage = event.validationException.message || '';
          if (errorMessage.includes('No open content found for content name')) {
            console.warn(`Received "No open content found" error for session ${sessionId}, this may be due to abrupt disconnection`);
      ...
      // Check for validation exception with "No open content found"
      const errorMessage = error instanceof Error ? error.message : String(error);
      if (errorMessage.includes('No open content found for content name')) {
        console.warn(`Caught "No open content found" error for session ${sessionId}, this may be due to abrupt disconnection`);
        // Mark session as inactive to prevent further operations
        if (session) {
          session.isActive = false;
        }
  ...
  public setupPromptStartEvent(sessionId: string, voiceId: string = 'tiffany'): void {
